...
img=cv2.imread("650.jpeg")#feed the test image in here
...
while(1):
    ...
    #this loop is to find the bottom right and right left point of the aruco marker    
    ...
    while(1):
        ...
        if(csum>cmax):
            cmax=csum
            x3=coordinates_list[0][0][0]
            y3=coordinates_list[0][0][1]
            k=o
        o+=1
    ...
    #the below loop is to find the top right and bottom left point of the aruco marker    
    x2=0
    y2=0
    x4=0
    y4=0
    ...
    while(1):
        ...
        cy=coordinates_list[0][0][1]
        csum=cx+cy
        if(abs(mean-csum)<tmin1 and o<k):
                tmin1=mean-csum
                x2=cx
                y2=cy
        if(abs(mean-csum)<tmin2 and o>k):        
                tmin2=mean-csum
                x4=cx
                y4=cy
        o+=1
        ...
        if(o==cp):
            break       
    #roi = img[y1:y3, x1:x3]#the cropped aruco marker is stored in this variable
    print(x1,y1)
    print(x2,y2)
    print(x3,y3)
    print(x4,y4)
    wA=np.sqrt(((x3-x2)**2)+((y3-y2)**2))
    wB=np.sqrt(((x4-x1)**2)+((y4-y1)**2))
    maxWidth = max(int(wA),int(wB))

    hA=np.sqrt(((x4-x3)**2)+((y4-y3)**2))
    hB=np.sqrt(((x1-x2)**2)+((y2-y2)**2))
    maxHeight = max(int(hA),int(hB))
    
    #now with the 4 points (x1,y1),(x2,y2),(x3,y3),(x4,y4) i will do perspective transformation.
    #i am doing perspective transformation to handle the case of rotated aruco markers
    point1=np.float32([[x1,y1],[x4,y4],[x3,y3],[x2,y2]])#corners of the image
    point2= np.float32([[0, 0],[maxWidth - 1, 0],[maxWidth - 1, maxHeight - 1],[0, maxHeight - 1]])
    A=cv2.getPerspectiveTransform(point1,point2)
    roi=cv2.warpPerspective(img,A,(maxWidth, maxHeight))
